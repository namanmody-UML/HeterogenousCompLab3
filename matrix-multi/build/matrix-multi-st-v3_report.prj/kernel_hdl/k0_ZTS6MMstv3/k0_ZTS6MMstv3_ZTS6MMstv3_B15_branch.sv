...
module k0_ZTS6MMstv3_ZTS6MMstv3_B15_branch (
    ...
    reg [0:0] c0_exe697460_reg_q;
    reg [0:0] c0_exe797563_reg_q;
    wire [0:0] c0_exe997767_cmp_q;
    reg [0:0] memdep_70_reg_q;
    ...
    // c0_exe997767_cmp(LOGICAL,6)
    assign c0_exe997767_cmp_q = ~ (in_c0_exe997767);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe997767_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe997767;
    ...
    // c0_exe697460_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe697460_reg_q <= $unsigned(1'b0);
        end
        else if (ZTS6MMstv3_B15_branch_enable_q == 1'b1)
        begin
            c0_exe697460_reg_q <= in_c0_exe697460;
        end
    end

    // out_c0_exe697460(GPOUT,21)
    assign out_c0_exe697460 = c0_exe697460_reg_q;

    // c0_exe797563_reg(REG,5)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe797563_reg_q <= $unsigned(1'b0);
        end
        else if (ZTS6MMstv3_B15_branch_enable_q == 1'b1)
        begin
            c0_exe797563_reg_q <= in_c0_exe797563;
        end
    end

    // out_c0_exe797563(GPOUT,22)
    assign out_c0_exe797563 = c0_exe797563_reg_q;

    // memdep_70_reg(REG,14)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            memdep_70_reg_q <= $unsigned(1'b0);
        end
        else if (ZTS6MMstv3_B15_branch_enable_q == 1'b1)
        begin
            memdep_70_reg_q <= in_memdep_70;
        end
    end

    // out_memdep_70(GPOUT,23)
    assign out_memdep_70 = memdep_70_reg_q;
